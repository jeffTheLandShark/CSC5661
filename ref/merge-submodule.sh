...
# script for converting submodules to directories while retaining file history from: https://github.com/jeremysears/scripts/blob/master/bin/git-submodule-rewrite
...
function main() {
    echo "Merging submodule ${sub} into parent repository"
    ...
    # Be sure to get all tags as well as we will merge them later on
    git fetch --tags
    pushd "${tmpdir}"
    local tab="$(printf '\t')"
    ...
    for submodule_tag in $(git tag --list --merged "${sub}/${branch}"); do
